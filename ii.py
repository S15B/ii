...
model_1.add(Dense(N_HIDDEN, Activation('relu'), Dropout(DROPOUT)))
model_1.add(Dense(NB_CLASSES, Activation('softmax')))
model_1.summary()
model_1.compile(loss='categorical_crossentropy', optimizer=SGD(learning_rate = 0.001), metrics=['accuracy'])

model_2 = Sequential()
model_2.add(Dense(N_HIDDEN * 2, Activation('elu'), input_shape=(RESHAPED,)))
model_2.add(Dense(N_HIDDEN * 2, Activation('elu'), Dropout(DROPOUT)))
model_2.add(Dense(NB_CLASSES, Activation('softmax')))
model_2.summary()
model_2.compile(loss='categorical_crossentropy', optimizer=SGD(learning_rate = 0.001), metrics=['accuracy'])
...
model_3.summary()
model_3.compile(loss='categorical_crossentropy', optimizer=RMSprop(learning_rate = 0.001), metrics=['accuracy'])
